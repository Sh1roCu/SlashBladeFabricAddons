...
public class EntityNRBlisteringSword extends absNeoSummonSword {
    //数据同步参数（移除TRACKING）
    ...
    protected int riseTicks = 0;
    protected int riseTick = 0;
    ...
    protected void defineSynchedData(SynchedEntityData.Builder builder) {
        ...
        builder.define(SPEED, 2.0F);
        builder.define(CENTER_POS, Vec3.ZERO.toVector3f());
        builder.define(DIRECTION_YAW, 0.0f);
        builder.define(DIRECTION_PITCH, 0.0f);
        builder.define(IT_CHANGED, true);
        builder.define(DIY_YAW, 0F);
        builder.define(DIY_PITCH, 0F);
        builder.define(LOCK_TARGET, Optional.empty());
        builder.define(LOCK_TARGET_ID, -1); // -1 表示无目标
    }
    ...
    // 上升阶段处理（子类可覆盖）
    protected void handleRisePhase() {
        riseTick++;
        // 基础实现：悬停不动
        setDeltaMovement(initialYaw, 0, initialPitch);
    }
    ...
    // 直接射线目标追踪
    private void trackRaycastTargetDirect() {
        Vec3 targetPos = calculateHitResultPosition();
        ...
        Vec3 newVelocity = directDirection.scale(currentSpeed);
    ...
    public static void spawnSwords(LivingEntity owner, Level world, Vec3 centerPos, SpawnMode mode, int count,
        ...
        for (int i = 0; i < count; i++) {
            EntityNRBlisteringSword sword = new EntityNRBlisteringSword(YAEntitiesRegistry.EntityNRBlisteringsword, world);
