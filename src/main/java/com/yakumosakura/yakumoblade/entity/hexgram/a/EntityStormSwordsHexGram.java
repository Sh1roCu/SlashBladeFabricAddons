 ...
 */
public class EntityStormSwordsHexGram extends absNeoSummonSword {
    // 同步字段：标记是否已发射（同螺旋飞剑）
    private static final EntityDataAccessor<Boolean> IT_FIRED;

    public EntityStormSwordsHexGram(EntityType<? extends Projectile> entityTypeIn, Level worldIn) {
    ...
    public void rideTick() {
        if (this.itFired()) {
            ...
            // 📌20 tick(1秒)后自动发射（区别于螺旋飞剑的10秒自毁）
