...
import com.yakumosakura.yakumoblade.entity.exer.SwordRainEntityLightningFather;
...
public class SwordRainLightning {
...
// 圆周参数
        ...
        for (int i = 0; i < count; i++) {
            ...
            ss.setColor(CapabilitySlashBlade.BLADESTATE.maybeGet(livingEntity.getMainHandItem())
...
// 圆周参数
        ...
        for (int i = 0; i < count; i++) {
            ...
            // 计算圆周坐标
            double angle = Math.toRadians(angleStep * finalI);
            double xOffset = radius * Math.cos(angle);
            double zOffset = radius * Math.sin(angle);
