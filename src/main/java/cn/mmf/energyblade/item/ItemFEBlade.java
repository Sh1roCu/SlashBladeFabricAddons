...
public class ItemFEBlade extends ItemSlashBlade implements IDamageable {
    ...
    public ItemFEBlade(Tier tier, int attackDamageIn, float attackSpeedIn, Properties builder) {
        ...
        EnergyStorage.ITEM.registerForItems((stack, context) -> {
            var bladeState = stack.get(CapabilitySlashBlade.BLADESTATE_COMPONENT);
            if (bladeState != null && bladeState.contains("Energy")) {
                var tag = bladeState.copyTag().getCompound("Energy");
                ...
                FEBladeStorage storage = new FEBladeStorage(energy, capacity, powerupExtract, standbyExtract, energyDurability) {
                    ...
                    protected void onFinalCommit() {
                        super.onFinalCommit();
                        CustomData.update(CapabilitySlashBlade.BLADESTATE_COMPONENT, stack, tag -> {
    ...
    @Override
    public <T extends LivingEntity> int damageItem(ItemStack arg0, int arg1, T arg2, Consumer<Item> arg3) {
        // TODO 电量耐久适配消耗
                ...
                if (bladeFE.isPowered()) {
                    try (Transaction tx = Transaction.openOuter()) {
                        if (bladeFE.extract(bladeFE.getStandbyExtract(), tx) == bladeFE.getStandbyExtract()) {
                            ...
                            CustomData.update(CapabilitySlashBlade.BLADESTATE_COMPONENT, event.getBlade(), tag -> {
